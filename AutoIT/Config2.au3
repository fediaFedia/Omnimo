...
#AutoIt3Wrapper_outfile=..\WP7\Common\Config\config2.exe
...
$Gui = GUICreate("Configure", $Size*2+10, $Size, $XPosition+5, $YPosition+5, BitOR($WS_VISIBLE,$WS_POPUP),$WS_EX_TOOLWINDOW)
...
$VariableInput = GUICtrlCreateInput("", 0, $Size/1.35-1, $Size*2+10, $Size/7.5, -1, 0)
...
$Set = GUICtrlCreateLabel("Set", $Size*1.85,  $Size/1.15, $Size/3, $Size/7.5)
GUICtrlSetColor(-1, 0x323232)
GUICtrlSetFont(-1, $Size/15, 600, 0, $font)
	...
	Switch $nMsg
		...
		Case $VariableList
			...
			If StringInStr($CurrentVarName, "Password", 2) Then
				...
				$VariableInput = GUICtrlCreateInput("", 0, $Size/1.35-1, $Size*2+10, $Size/7.5, $ES_PASSWORD, 0)
			Else
				GUICtrlDelete($VariableInput)
				$VariableInput = GUICtrlCreateInput("", 0, $Size/1.35-1, $Size*2+10, $Size/7.5, -1, 0)
			EndIf
