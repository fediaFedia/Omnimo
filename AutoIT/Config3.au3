...
#AutoIt3Wrapper_outfile=..\WP7\Common\Config\config3.exe
...
$Gui = GUICreate("Configure", $Size, $Size*2+10, $XPosition+5, $YPosition+5, BitOR($WS_VISIBLE,$WS_POPUP),$WS_EX_TOOLWINDOW)
GUISetBkColor(0xe1e1e1)
GUICtrlCreatePic("header.jpg", $Size/30, 0, $Size/3.75, $Size/30, Default)
$VariableList = GUICtrlCreateList("", 10, 15, $Size/1.07, $Size/5*8.6, BitOR($ES_AUTOVSCROLL,$ES_AUTOHSCROLL,$WS_HSCROLL,$WS_VSCROLL), 0)
GUICtrlSetBkColor(-1, 0xe1e1e1)
GUICtrlSetColor(-1, 0x323232)
GUICtrlSetFont(-1, $Size/15, 400, 0, $font)
$VariableInput = GUICtrlCreateInput("", 0, $Size*1.78, $Size, $Size/7.5, -1, 0)
...
$Set = GUICtrlCreateLabel("Set", $Size/1.35,  $Size*1.92, $Size/3, $Size/7.5)
GUICtrlSetColor(-1, 0x323232)
GUICtrlSetFont(-1, $Size/15, 600, 0, $font)
	...
	Switch $nMsg
		...
		Case $VariableList
			...
			If StringInStr($CurrentVarName, "Password", 2) Then
				...
				$VariableInput = GUICtrlCreateInput("", 0, $Size*1.78, $Size, $Size/7.5, BitOR($ES_AUTOHSCROLL, $ES_PASSWORD), 0)
			Else
				GUICtrlDelete($VariableInput)
				$VariableInput = GUICtrlCreateInput("", 0, $Size*1.78, $Size, $Size/7.5, -1, 0)
			EndIf
